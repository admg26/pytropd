...
def TropD_Metric_EDJ(
    ...
    **kwargs : optional
    ...
    if method != "fit":  # max or peak
        ...
        return Phi*hem, Umax  # type: ignore
    ...
    **kwargs : optional
    ...
    if method != "fit":
        ...
        if "adjusted" in method:  # adjusted_max or adjusted_peak
            ...
            lat_before_EDJ = lat[mask] < PhiEDJ[..., None]
...
def TropD_Metric_UAS(
    ...
    uas_min_lat = TropD_Calculate_MaxLat(
        -uas[..., subpolar_mask], lat[subpolar_mask]
    )
...
def Shah_2020_GWL(
    ...
    grad_weight_lat = (
        np.diff(tracer_lat, axis=-1) / np.diff(phi_lat) * np.cos(phi_mid_lat)
    )
...
def Shah_2020_1sigma(
    ...
    sigma_width_lat = (
        np.ma.masked_where(
            ~((tracer_lat < threshold) & (phi_lat > phi_lat[..., :1])),
            phi_lat,
            copy=False,
        )
        .min(axis=-1)
